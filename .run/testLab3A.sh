...
#a=$(VERBOSE=1 go test -race -run Speed3A > ee.log && tail -n 1 ee.log | grep -o ok)
a=$(VERBOSE=0 go test -race -run PersistConcurrent3A > ee.log && tail -n 1 ee.log | grep -o ok)
